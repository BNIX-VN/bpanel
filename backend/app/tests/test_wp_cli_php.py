...
def test_the_panel_logger_actually_has_somewhere_to_write():
    ...
    Everything the panel logs goes to logging.getLogger("bpanel"), which
    ...
    assert "bpanel" in config["loggers"], "the panel's logger is not configured"
    assert config["loggers"]["bpanel"]["handlers"], "configured, but with no handler"
