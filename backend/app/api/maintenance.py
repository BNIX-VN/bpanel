...
from app.schemas.schemas import BackupCreate, CronCreate, CronDelete, PhpConfigUpdate, RestoreBackup, WpAction
from app.services import backup, cron, file_manager, php, wordpress
...
def _is_secure_request(request: Request) -> bool:
    ...
    return forwarded_proto == "https" or request.url.scheme == "https" or settings.app_env == "production"
...
def wordpress_action(payload: WpAction, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    website = get_owned_website(db, current_user, payload.website_id)
    result = wordpress.wp_update(f"{website.root_path}/public", payload.action)
...
def fix_wordpress_permissions(website_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    website = get_owned_website(db, current_user, website_id)
    wordpress.fix_permissions(website.root_path)
