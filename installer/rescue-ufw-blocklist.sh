#!/usr/bin/env bash
# Emergency recovery for BPanel hosts that became unreachable after importing
...
ts="$(date +%Y%m%d-%H%M%S)"
backup_dir="/root/bpanel-ufw-rescue-${ts}"
...
ufw allow OpenSSH comment "bpanel:PanelZone" || ufw allow OpenSSH || true
ufw allow "${ssh_port}/tcp" comment "bpanel:PanelZone" || ufw allow "${ssh_port}/tcp" || true
for port in "$panel_port" 80 443 465 587; do
  [[ "$port" =~ ^[0-9]{1,5}$ ]] || continue
  ufw allow "${port}/tcp" comment "bpanel:PanelZone" || ufw allow "${port}/tcp" || true
done
